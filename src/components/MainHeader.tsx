...
const MainHeader = () => {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 w-full z-50 font-sans transition-transform duration-300 ${
      ...
      >
        ...
        <div className="bg-pascalBlue text-white text-xs sm:text-sm">
          <div className="flex justify-between items-center max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-10">
            <div className="bg-pascalRed px-3 py-2 font-semibold whitespace-nowrap">
              LATEST NEWS
            </div>
        ...
        <div className="bg-pascalRed text-white text-xs sm:text-sm py-2">
          <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 flex flex-col md:flex-row justify-between items-center space-y-2 md:space-y-0">
            ...
            <div className="flex items-center gap-3 text-sm sm:text-base">
              ...
              {[FaWhatsapp, FaFacebookF, FaTwitter, FaGooglePlusG, FaLinkedinIn, FaYoutube, FaPinterestP].map(
                (Icon, i) => (
                  <Icon
                    key={i}
          ...
          <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              <Image
                ...
                width={70}
                height={70}
                className="w-16 sm:w-20 md:w-[90px] h-auto"
                priority
              />
                        ...
                        <ul className="absolute top-full left-0 w-64 bg-white shadow-md z-50">
                          {academicDropdownItems.map((subItem) => (
                            <li key={subItem.name}>
                              <Link
                                href={subItem.href}
                                className={`block px-4 py-2 hover:bg-pascalRed hover:text-white ${
                            ...
                            <ul className="pl-4 space-y-1">
                              {academicDropdownItems.map((subItem) => (
                                <li key={subItem.name}>
                                  <Link
                                    href={subItem.href}
                                    className={`block py-2 px-2 hover:bg-pascalRed hover:text-white ${
                                      pathname === subItem.href
                                        ? "bg-pascalRed text-white"
                                        : ""
                                    }`}
