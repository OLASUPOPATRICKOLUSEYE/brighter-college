...
type FormData = z.infer<typeof studentCategorySchema>;

export default function StudentCategoryForm({
  type,
  data,
  onClose,
  onSuccess,
  ...
  const onSubmit = async (formData: FormData) => {
    setLoading(true);
    const toastId = toast.loading(type === "create" ? "Student Category Submitting..." : "Student Category Updating...");

    try {
      const res = await fetch(
        type === "create" ? "/api/studentcategory" : `/api/studentcategory/${data?._id}`,
        {
      ...
      toast.success(
        type === "create"
          ? "Student Category Submitted Successfully!"
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col text-black space-y-4 text-sm">
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Student Category" : "Update Student Category"}
      </h1>

      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-5 pt-4">
        {/* <InputField label="Category ID *" name="categoryId" register={register} error={errors.categoryId}/>        */}
        <InputField label="Category *" name="category" register={register} error={errors.category} /> 
        {/* Description */}
