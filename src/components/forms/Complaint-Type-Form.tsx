...
type FormData = z.infer<typeof complaintTypeSchema>;
...
export default function ComplaintTypeForm({
  ...
  const onSubmit = async (formData: FormData) => {
    setLoading(true);
    const toastId = toast.loading(type === "create" ? "Complaint Type Submitting..." : "Complaint Type Updating...");

    try {
      const res = await fetch(
        type === "create" ? "/api/complainttype" : `/api/complainttype/${data?._id}`,
        {
      ...
      toast.success(
        type === "create"
          ? "Complaint Type Submitted Successfully!"
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col text-black space-y-4 text-sm">
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Add Complaint Type" : "Update Complaint Type"}
      </h1>

      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-5 pt-4">
        {/* <InputField label="Complaint Type ID *" name="complainttypeId" register={register} error={errors.complainttypeId} />         */}
        <InputField label="Complaint Type *" name="complainttype" register={register} error={errors.complainttype} />                
        {/* Description */}
