...
import { hostelSchema } from "@/lib/validation/validationSchemas";
import InputField from "../InputField";

type FormData = z.infer<typeof hostelSchema>;
...
export default function HostelForm({
  ...
  const onSubmit = async (formData: FormData) => {
    setLoading(true);
    const toastId = toast.loading(type === "create" ? "Hostel Submitting..." : "Hostel Updating...");

    try {
      const res = await fetch(
        type === "create" ? "/api/hostel" : `/api/hostel/${data?._id}`,
        {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col text-black space-y-4 text-sm">
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Hostel" : "Update Hostel"}
      </h1>

      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-5 pt-4">
        <InputField label="Hostel Name*" name="hostelName" register={register} error={errors.hostelName} />      
        {/* Description */}
