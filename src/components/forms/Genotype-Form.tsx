...
type FormData = z.infer<typeof genotypeSchema>;

export default function GenotypeForm({
  type,
  data,
  onClose,
  onSuccess,
  ...
  const {
    ...
    resolver: zodResolver(genotypeSchema),
    defaultValues: {},
  });
  ...
  const onSubmit = async (formData: FormData) => {
    setLoading(true);
    const toastId = toast.loading(type === "create" ? "Genotype Submitting..." : "Genotype Updating...");

    try {
      const res = await fetch(
        type === "create" ? "/api/genotype" : `/api/genotype/${data?._id}`,
        {
      ...
      toast.success(
        type === "create"
          ? "Genotype Submitted Successfully!"
          : "Genotype Updated Successfully!",
        { id: toastId }
      );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col text-black space-y-4 text-sm">
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Genotype" : "Update Genotype"}
      </h1>

      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-5 pt-4">
        {/* <InputField label="Genotype ID *" name="genotypeId" register={register} error={errors.genotypeId} /> */}
        <InputField label="Genotype *" name="genotype" register={register} error={errors.genotype} />      
        {/* Description */}
