...
type FormData = z.infer<typeof studentHouseSchema>;

export default function StudentHouseForm({
  type,
  data,
  onClose,
  onSuccess,
  ...
  const {
    ...
    resolver: zodResolver(studentHouseSchema),
    defaultValues: {},
  });
  ...
  const onSubmit = async (formData: FormData) => {
    setLoading(true);
    const toastId = toast.loading(type === "create" ? "Student House Submitting..." : "Student House Updating...");

    try {
      const res = await fetch(
        type === "create" ? "/api/studenthouse" : `/api/studenthouse/${data?._id}`,
        {
      ...
      toast.success(
        type === "create"
          ? "Student House Submitted Successfully!"
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col text-black space-y-4 text-sm">
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Student House" : "Update Student House"}
      </h1>

      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-5 pt-4">
        {/* <InputField label="House ID *" name="houseId" register={register} error={errors.houseId} />       */}
        <InputField label="Student House Name *" name="studenthouse" register={register} error={errors.studenthouse} />  
        {/* Description */}
