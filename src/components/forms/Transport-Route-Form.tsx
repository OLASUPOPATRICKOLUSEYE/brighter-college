...
type FormData = z.infer<typeof transportRouteSchema>;
...
export default function TransportRouteForm({
  ...
  const onSubmit = async (formData: FormData) => {
    setLoading(true);
    const toastId = toast.loading(type === "create" ? "Transport Route Submitting..." : "Transport Route Updating...");

    try {
      const res = await fetch(
        type === "create" ? "/api/transportroute" : `/api/transportroute/${data?._id}`,
        {
      ...
      toast.success(
        type === "create"
          ? "Transport Route Submitted Successfully!"
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col text-black space-y-4 text-sm">
      <h1 className="text-xl font-semibold">
        {type === "create" ? "Transport Route" : "Update Transport Route"}
      </h1>

      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-5 pt-4">
        <InputField label="Transport Route *" name="routeName" register={register} error={errors.routeName} />      
        {/* Description */}
