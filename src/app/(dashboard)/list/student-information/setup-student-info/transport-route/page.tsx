...
const TransportRoute = () => {
  ...
  const fetchTransportRoutes = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/transportroute?${queryParams.toString()}`);
      if (!res.ok) throw new Error("Transport Route Not Found");
      const data = await res.json();

      setTransportRoutes(data.data || []);
  ...
  return (
    <div className="bg-white rounded-md flex-1">
      <div className="flex px-4 pt-4 flex-col md:flex-row md:justify-between mb-4 gap-2 md:gap-0">
        <h1 className="text-lg font-semibold">All Transport Route</h1>
        <div className="flex flex-col sm:flex-row gap-2 items-center">
          <TableSearch value={searchTerm} onChange={setSearchTerm} />
            <div className="flex items-center gap-4 self-center flex flex-wrap justify-center">
              ...
              { isAdmin && (
                <FormModal table="transportroute" type="create" onSuccess={handleSuccess} />
              )}
            </div>
        </div>
      </div>
          ...
          <table className="w-full border-collapse mt-4 text-sm">
            <thead>
                  <tr className="text-left text-gray-500">
                    ...
                    >
                      Transport Route ID{" "}
                      <span className={sortBy === "routeId" ? "text-black" : "text-gray-300"}>
                        {sortOrder === "asc" ? "↑" : "↓"}
                      </span>
                    </th>
                    <th
                    ...
                    >
                      Transport Route{" "}
                      <span className={sortBy === "routeName" ? "text-black" : "text-gray-300"}>
                        {sortOrder === "asc" ? "↑" : "↓"}
                      </span>
                    </th>
                    <th
            ...
            {loading && (
              <tr>
                <td colSpan={colCount} className="p-6 text-center">
                  <TableLoading message="Fetching Transport Route..." />
                </td>
              </tr>
            )}
            ...
            {!loading && !error && transportRoutes.length === 0 && (
              <tr>
                <td colSpan={colCount} className="p-6 text-center">
                  <TableNotFound message="No Transport Route Available." />
                </td>
              </tr>
            )}
            ...
            {!loading && !error &&
              transportRoutes.map((item) => (
                ...
                >
                  <td className="p-4 break-words">{item.routeId}</td>
                  <td className="p-4 break-words">{item.routeName}</td>
                  <td className="p-4 break-words">{item.description}</td>
                  { isAdmin && (
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      <FormModal table="transportroute" type="view" data={item} onSuccess={handleSuccess} />
                      <FormModal table="transportroute" type="update" data={item} onSuccess={handleSuccess} />
                      <FormModal table="transportroute" type="delete" id={item._id} onSuccess={handleSuccess} />
                    </div>
                  </td>
                  )}
                </tr>
