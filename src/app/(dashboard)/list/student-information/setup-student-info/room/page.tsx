...
const Room = () => {
  ...
  const fetchRooms = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/room?${queryParams.toString()}`);
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-center">
          <TableSearch value={searchTerm} onChange={setSearchTerm} />
            <div className="flex items-center gap-4 self-center">
              ...
              { isAdmin && (
                <FormModal table="room" type="create" onSuccess={handleSuccess} />
              )}
            </div>
        </div>
      </div>
          ...
          <table className="w-full border-collapse mt-4 text-sm">
            <thead>
                  <tr className="text-left text-gray-500">
                    ...
                    >
                      Room ID{" "}
                      <span className={sortBy === "roomId" ? "text-black" : "text-gray-300"}>
                        {sortOrder === "asc" ? "↑" : "↓"}
                      </span>
                    </th>
                    <th
                    ...
                    >
                      Room Name{" "}
                      <span className={sortBy === "roomName" ? "text-black" : "text-gray-300"}>
                        {sortOrder === "asc" ? "↑" : "↓"}
                      </span>
                    </th>
                    <th
            ...
            {loading && (
              <tr>
                <td colSpan={colCount} className="p-6 text-center">
                  <TableLoading message="Fetching Rooms..." />
                </td>
              </tr>
            )}
            ...
            {!loading && !error && rooms.length === 0 && (
              <tr>
                <td colSpan={colCount} className="p-6 text-center">
                  <TableNotFound message="No Room Available." />
                </td>
              </tr>
            )}
            ...
            {!loading && !error &&
              rooms.map((item) => (
                ...
                >
                  <td className="p-4 break-words">{item.roomId}</td>
                  <td className="p-4 break-words">{item.roomName}</td>
                  <td className="p-4 break-words">{item.description}</td>
                  { isAdmin && (
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      <FormModal table="room" type="view" data={item} onSuccess={handleSuccess} />
                      <FormModal table="room" type="update" data={item} onSuccess={handleSuccess} />
                      <FormModal table="room" type="delete" id={item._id} onSuccess={handleSuccess} />
                    </div>
                  </td>
                  )}
                </tr>
